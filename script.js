...
const setStyles = () => {
  ...
  for (let i = 0; i < els.length; i++) {
    els[i].querySelector(
      INNER_SELECTOR
    ).style.transform = `translateZ(-${PROJECTION}px) rotateY(0deg)`;
  }
};

const getLetterMarkup = (letter) =>
  `<div class="letterWrap"><div class="letter"><span>${letter}</span></div></div>`;
